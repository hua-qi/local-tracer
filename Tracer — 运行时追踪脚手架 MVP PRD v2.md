# @toft/local-runtime-tracer — 运行时追踪脚手架 MVP PRD v2

## 1. 概述

### 1.1 产品定位

**@toft/local-runtime-tracer 是一个纯本地开发调试工具，辅助 Code Agent 在前端项目中定位复杂 bug。**
...
- **生产环境、线上部署、远端上报——@toft/local-runtime-tracer 是纯本地工具**
...
cp node_modules/@toft/local-runtime-tracer/skill.md .claude/skills/tracer.md
```
...
# 2. 配置构建工具（二选一，本地配置文件）
# vite.config.ts:
import { tracerVitePlugin } from '@toft/local-runtime-tracer/vite'
export default { plugins: [tracerVitePlugin()] }

# webpack.config.js:
const { TracerWebpackPlugin } = require('@toft/local-runtime-tracer/webpack')
