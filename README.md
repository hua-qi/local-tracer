...
| | local-runtime-tracer | Sentry / Datadog / 自建埋点 |
|---|---|---|
...
local-runtime-tracer 不面向生产环境，不上报任何数据到远端。唯一的使用场景是：开发者在本地 `npm run dev`，Code Agent 借助运行时日志来理解代码的实际执行情况，从而更精准地定位**需要多轮迭代才能修复的复杂 bug**。
...
| | local-runtime-tracer | Browser DevTools MCP 类 |
|---|---|---|
...
```bash
cp node_modules/local-runtime-tracer/skill.md .claude/skills/local-runtime-tracer.md
```
...
│       └─ {traceId}.jsonl         ← 运行时日志
│
└─ node_modules/local-runtime-tracer/
    ├── src/
    │   ├── core/
