...
| | @toft/local-tracer | Sentry / Datadog / 自建埋点 |
|---|---|---|
...
@toft/local-tracer 不面向生产环境，不上报任何数据到远端。唯一的使用场景是：开发者在本地 `npm run dev`，Code Agent 借助运行时日志来理解代码的实际执行情况，从而更精准地定位**需要多轮迭代才能修复的复杂 bug**。
...
| | @toft/local-tracer | Browser DevTools MCP 类 |
|---|---|---|
...
```bash
npm install -D @toft/local-tracer
```
...
```ts
import { huaqiFEVitePlugin } from '@toft/local-tracer/vite'
...
```bash
cp node_modules/@toft/local-tracer/skill.md .claude/skills/@toft/local-tracer.md
```
...
│       └─ {traceId}.jsonl         ← 运行时日志
│
└─ node_modules/@toft/local-tracer/
    ├── src/
    │   ├── core/
