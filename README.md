...
| | huaqi-FE-tracer | Sentry / Datadog / 自建埋点 |
|---|---|---|
...
huaqi-FE-tracer 不面向生产环境，不上报任何数据到远端。唯一的使用场景是：开发者在本地 `npm run dev`，Code Agent 借助运行时日志来理解代码的实际执行情况，从而更精准地定位**需要多轮迭代才能修复的复杂 bug**。
...
| | huaqi-FE-tracer | Browser DevTools MCP 类 |
|---|---|---|
...
```bash
cp node_modules/huaqi-FE-tracer/skill.md .claude/skills/huaqi-fe-tracer.md
```
