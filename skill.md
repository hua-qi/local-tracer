...
| `traces[].type` | enum | 是 | `api_call` \| `api_response` \| `state_change`（MVP） |
...
| 表达式 | 含义 | 适用于 |
|---|---|---|
| `arguments[N]` | 匹配调用的第 N 个参数 | `api_call` |
...
- **`callStack` → 调用路径**：确认哪个调用者触发了该函数。如果 `fetchUserData` 意外地从 `onMount` 调用而非 `handleLogin`，那就是 bug。
